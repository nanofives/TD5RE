...
| G3 | td5_game.h migration + extern paydown | wip fix-refactor-g3-ai-externs | SLICE 6 (in progress): td5_ai.c extern paydown 6->0. 2 redundant deletes (td5_sound_cop_siren_is_on@sound.h, td5_physics_reset_actor_state@physics.h -- both included); 1 add-include (td5_net_is_slot_active already in td5_net.h, ai.c gains the include); 1 add-to-header (g_route_data -> td5_game.h); 2 add-to-header+include (g_wanted_msg_timer/index -> td5_hud.h, ai.c gains the include -- AI writes the HUD wanted-message state, pre-existing coupling now visible in headers). Pure decl relocation; build+lint verifies, no launch. PRIOR: hud 22->0, game 14->2, vfx 8->0, camera 7->0, extern_in_c_total 103->52
