...
| G3 | td5_game.h migration + extern paydown | done (partial) @3dd520ca | extern paydown 6 slices, extern_in_c_total 103->46: hud.c 22->0, game.c 14->2, vfx.c 8->0, camera.c 7->0, ai.c 6->0 (@3dd520ca). All pure declaration relocation into producer headers, zero emitted-code change, build+lint-verified, NO game launch. game.c's 2 remaining LEFT ON PURPOSE (frontend layering smell + 3rd-party stb). Remaining: render_mesh/render/render_effects 5 each, physics 4, input 4, physics_collision/render_pipeline/ai_traffic/platform_win32_window 3 each, misc smaller + the broader td5_game.h->td5_race_state.h includer migration -- same build+lint-only pattern
