...
| G3 | td5_game.h migration + extern paydown | wip fix-refactor-g3-game-externs | SLICE 3 (in progress): td5_game.c extern paydown (the plan's "game 16" target; concurrent merge already trimmed it to 14). 7 pure-redundant deletes (already in included headers: 3 camera globals from slice 2, g_render_width_f/height_f@render.h, g_track_is_circuit/type_mode@track.h), 5 add-to-header+delete (g_cameraPos+g_trackType→td5_camera.h, g_track_environment_config→td5_asset.h, td5_net_host/client_frame_nb→td5_net.h), 2 intentionally LEFT (frontend_mp_simul_preview_setup = game→frontend layering smell w/ no clean header; stbi_write_png = third-party stb). game.c 14→2. Pure decl relocation; build+lint verifies, no launch. PRIOR: td5_hud.c extern paydown COMPLETE (was the plan's "hud 22" first target -> now 0). Slice 1 @abdfe319: 8 function externs. Slice 2 @6aa6205c: 14 global-variable externs -> producer headers (3 td5_camera.h, 5 td5_game.h, 2 td5_ai.h, 2 already-in-header redundant deletes, 2 hud.c-local self-extern deletes). All pure declaration relocation, zero emitted-code change. extern_in_c_total 103->81 across both slices; baseline JSON ratcheted down each time (also locked in S7's game_h_includers 25->24). Build dev+release clean, no new warnings, structure lint OK. Verified by build + lint ratchet alone -- NO game launch (declaration-only; also a concurrent manual_drive held the GPU). Left for follow-up: td5_game.c's 16 externs (the plan's "game 16" second target) + the broader td5_game.h->td5_race_state.h includer migration -- same build+lint-only verification applies |
