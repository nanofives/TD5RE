...
| C1 | Coverage build-out (camera/sound traces, save/net tests) | done (partial) @789b9f0b | net-loopback promotion SHIPPED: net-loopback verdict row hosts a DIRECT session in-process + spawns a child td5re.exe (TD5RE_NET_SELFTEST=join env; child exits pre-Backend_CreateDevice = no GPU/TDR hazard), pumps the UDP handshake to player_count>=2, PID-scoped kill on timeout. First automated net coverage in the suite. PASS (host=1 spawn=1 handshake=1) on 2 consecutive full runs; trace-goldens matched both. Env noise those runs (documented GPU device-loss 0x887A0005 -> render-capture WARNs + ~6x frame-time FAIL) unrelated to this report-phase-only step. LESSON: 2 earlier runs wasted on a worktree missing re/assets (placeholder-track fallback exits cleanly mid-suite w/ no report, mimics flakiness) -- worktree_setup.ps1 is MANDATORY for any game-launching slice. ALSO SHIPPED: save/load round-trip @d5adf821. REMAINING: camera/sound trace streams + goldens (next); drag natural-finish + harness order bug stay their own package |
