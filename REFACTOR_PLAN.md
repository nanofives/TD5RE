...
| G3 | td5_game.h migration + extern paydown | wip fix-refactor-g3-sweep-externs | SLICE 8 (in progress, final sweep): clear ALL remaining paydownable extern-in-.c (31 -> 3 intentional). main.c 3 (win32 bootstrap fns -> td5_platform.h), ai_traffic 2 of 3 (branch-corridor fns redundant@track.h; the 3rd is a deliberate __attribute__((weak)) linkage extern -- STAYS), input.c 4 (2 redundant@game.h + AngleFromVector12@render.h/get_player_manual@frontend.h via new includes), physics family 11 (g_actor_base+g_actor_table_base -> td5_physics_internal.h to avoid raising the game_h_includers ratchet on collision/drivetrain/suspension; hull_build_store -> physics_internal; 2 redundant in physics.c via game.h; ai_get_route_state redundant@ai.h), window.c 3 (render dims -> td5_render.h include; td5re_set_render_dims -> td5re.h + include), render_pipeline 3 (depthFov/projFov -> camera.h; td5_trig_lut_bits -> NEW tiny td5_trig_lut_data.h producer header), td5re.c 2 (redundant@render.h). End state: extern_in_c_total = 3, all intentional (game.c frontend-layering + stb, ai_traffic weak). PRIOR: 103->31 over 7 slices
