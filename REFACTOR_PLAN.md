...
| C2 | td5_camera.c cleanup | done (partial) @8a2e1f30 | SLICES 1+2 DONE: FP_* macro migration (30 shifts @d515bf70) + all 6 gotos -> structured control flow (@8a2e1f30: fly-in counter skip inverted into each branch; countdown preset-reload guarded by a do_load flag). Both behavior-preserving; camera golden-guarded via C1 camera trace -> both scenarios '7 module hashes match goldens' on each slice. Build clean. ONLY REMAINING C2: name the preset offsets (comment/naming work, zero behavior) -- optional follow-up slice
