...
| G3 | td5_game.h migration + extern paydown | done (partial) @fe9d14ca | extern paydown 7 slices, extern_in_c_total 103->31: hud.c 22->0, game.c 14->2, vfx.c 8->0, camera.c 7->0, ai.c 6->0, render/render_mesh/render_effects 5+5+5->0 (@fe9d14ca; also fixed a stale mismatched-bounds extern -- g_raceCameraPresetMode was locally declared [2] vs real [TD5_MAX_VIEWPORTS]). All pure declaration relocation into producer headers, zero emitted-code change, build+lint-verified, NO game launch. game.c's 2 remaining LEFT ON PURPOSE (frontend layering smell + 3rd-party stb). Remaining ~31: physics 4, input 4, physics_collision/render_pipeline/ai_traffic/platform_win32_window 3 each, main.c 3, misc 2s + the broader td5_game.h->td5_race_state.h includer migration -- same build+lint-only pattern
