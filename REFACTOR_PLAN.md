...
| G3 | td5_game.h migration + extern paydown | done (partial) @836c6591 | extern paydown: td5_hud.c COMPLETE (22->0, slices @abdfe319 + @6aa6205c) + td5_game.c 14->2 (@836c6591). extern_in_c_total 103->67 across 3 slices; baseline ratcheted down each. All pure declaration relocation (globals/functions moved from extern-in-.c into producer headers: td5_camera/game/track/ai/render/asset/net .h), zero emitted-code change, build+lint-verified, NO game launch. game.c's 2 remaining externs LEFT ON PURPOSE (frontend_mp_simul_preview_setup = game->frontend layering smell; stbi_write_png = 3rd-party stb). Remaining G3 work: the other modules' extern-in-.c (vfx 8, camera 7, ai 6, render* 5+5+5+3, input 4, physics* etc) + the broader td5_game.h->td5_race_state.h includer migration -- all same build+lint-only pattern |
