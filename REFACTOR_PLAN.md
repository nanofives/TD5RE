...
| G3 | td5_game.h migration + extern paydown | wip fix-refactor-g3-render-externs | SLICE 7 (in progress): render trio extern paydown (render.c 5 + render_mesh.c 5 + render_effects.c 5 -> 0). 8 redundant deletes (7x g_subTickFraction@camera.h, g_tick_counter@game.h -- all included); 3 add-to-header+delete (g_cameraSecondaryUnscaled x2 + g_raceCameraPresetMode -> td5_camera.h; NOTE the old local extern declared it [2] but the real def is [TD5_MAX_VIEWPORTS]=9 -- stale mismatched-bounds extern, header decl fixes it); 4 add-include deletes (td5_input_is_playback_active@input.h x2, td5_sound_siren_is_enabled@sound.h, td5_hud_update_wanted_damage_indicator@hud.h -- all already declared, the 3 files gain the includes). Pure decl relocation; build+lint, no launch. PRIOR: hud 22->0, game 14->2, vfx 8->0, camera 7->0, ai 6->0, total 103->46
