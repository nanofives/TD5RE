...
| G3 | td5_game.h migration + extern paydown | done (partial) @c2c381b5 | extern paydown 4 slices, extern_in_c_total 103->59: hud.c 22->0 (@abdfe319+@6aa6205c), game.c 14->2 (@836c6591), vfx.c 8->0 (@c2c381b5). All pure declaration relocation into producer headers (camera/game/track/ai/render/asset/net .h), zero emitted-code change, build+lint-verified, NO game launch. game.c's 2 remaining LEFT ON PURPOSE (frontend layering smell + 3rd-party stb). Remaining: camera.c 7, ai.c 6, render_mesh/render/render_effects 5 each, input 4, physics* etc + the broader td5_game.h->td5_race_state.h includer migration -- same build+lint-only pattern
