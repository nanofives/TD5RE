...
| C9 | td5_platform_win32.c split | done (partial) @995d1017 | third slice DONE: extracted Audio (DirectSound8) + CD Audio (MCI) + Radio PCM streaming (~1360 lines, 39 fns) into td5_platform_win32_audio.c, plus their module-level statics. 13 of those statics (buffer/channel tables, alloc counters) stayed shared via td5_platform_internal.h (extended w/ dsound.h+stdint.h) since td5_platform_win32_init()'s reset block still touches them -- everything else moved wholesale. First two slices DONE: filesystem @eb6f7ccf, threading+logging @94d25efc. All three slices: build+lint clean (same pre-existing unrelated td5_ai_traffic.c extern note from S5), full suite 46/46 PASS, goldens matched (sound-options/music-test screens + all race audio exercised throughout). Left undone: window/input, render-preset sections -- larger, entangled with D3D11 wrapper's g_backend state, expect S5/S6-scope internal-header work |
