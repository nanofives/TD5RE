...
| G3 | td5_game.h migration + extern paydown | done (partial) @a8fbff1c | extern paydown 5 slices, extern_in_c_total 103->52: hud.c 22->0, game.c 14->2, vfx.c 8->0, camera.c 7->0 (@a8fbff1c). All pure declaration relocation into producer headers (camera/game/track/ai/render/asset/net/physics .h), zero emitted-code change, build+lint-verified, NO game launch. game.c's 2 remaining LEFT ON PURPOSE (frontend layering smell + 3rd-party stb). Remaining: ai.c 6, render_mesh/render/render_effects 5 each, input 4, physics_collision 3, render_pipeline 3, ai_traffic 3, platform_win32_window 3, physics 4, misc + the broader td5_game.h->td5_race_state.h includer migration -- same build+lint-only pattern
