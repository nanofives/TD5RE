...
| C9 | td5_platform_win32.c split | wip fix-refactor-c9-platform-window | fourth slice: Window/Display + Timing (~810 lines). Pre-claim investigation found ~11 shared symbols (window dims/mode/fullscreen/qpc/wndproc-subclass/s_primary, all one-directional Init->window.c, same extern pattern as slice 3) + one reverse-direction dependency (td5_plat_present reads rendering-backend's texture/frame-stat globals) + icon-helper cross-refs (icon section not yet extracted) -- tractable, not S5-AI-traffic-scale. First 3 slices DONE: filesystem @eb6f7ccf, threading+logging @94d25efc, audio-device @995d1017 |
