...
| C1 | Coverage build-out (camera/sound traces, save/net tests) | wip fix-refactor-c1-netloopback | net-loopback promotion (in progress): new net-loopback verdict step in td5_selftest.c -- suite process hosts a DIRECT session in-process (lobby pattern: td5_net_shutdown->init->set_mode->create_session_ex), spawns a CHILD td5re.exe with TD5RE_NET_SELFTEST=join (existing dev harness; child exits at main.c:1022 BEFORE Backend_CreateDevice -> no window/GPU -> no multi-instance TDR hazard), pumps td5_net_tick until player_count>=2 or timeout, kills child by OWN HANDLE only on timeout (PID-scoped rule). PRIOR: save/load round-trip SHIPPED @d5adf821 (49/49 PASS)
