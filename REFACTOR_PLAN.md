...
| G3 | td5_game.h migration + extern paydown | wip fix-refactor-g3-hud-externs | first slice: extern paydown of td5_hud.c's 8 FUNCTION externs (the clean, unambiguous subset of its 22) -- td5_ai_get_route_state (already in td5_ai.h+included -> pure delete), td5_camera_apply_view/get_basis (already in td5_camera.h -> add include+delete), 3x td5_game_* + 2x td5_track_* (add decls to producer headers -> delete externs). Pure declaration relocation, zero emitted-code change -> verified by build+lint ratchet-down alone, NO game launch needed (chosen deliberately: a concurrent manual_drive session is holding the GPU). Owns choke files td5_game.h + lint baseline JSON. Leaves hud.c's ~14 global-variable externs (g_*, trickier -- no clean producer headers) + game.c's 16 for follow-up slices. S5/S7 deps both done |
