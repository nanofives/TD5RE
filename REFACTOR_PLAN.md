...
| G3 | td5_game.h migration + extern paydown | wip fix-refactor-g3-vfx-externs | SLICE 4 (in progress): td5_vfx.c extern paydown 8->0. 4 redundant deletes (g_subTickFraction@camera.h, g_track_environment_config@asset.h, td5_render_submit_translucent_world/tire_mark@render.h -- all already in vfx-included headers); 4 add-to-header+delete (g_worldToRenderScale + g_cameraBasis -> td5_camera.h, g_actor_table_base -> td5_game.h, td5_render_set_tire_mark_fx_preset -> td5_render.h). Pure decl relocation; build+lint verifies, no launch. PRIOR: hud.c 22->0, game.c 14->2, extern_in_c_total 103->67
