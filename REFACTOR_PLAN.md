...
| C1 | Coverage build-out (camera/sound traces, save/net tests) | wip fix-refactor-c1-saveload-roundtrip | taking just the save/load round-trip sub-item (independent of the reverted drag-finish fix and the still-open harness order-sensitivity bug -- both untouched). Found td5_save_write_cup_data/load_cup_data already accept an explicit path "used by the dev-only roundtrip self-test" per their own doc comments -- designed for this, never wired in. Adding a synchronous save-load-roundtrip check to td5_selftest.c's report phase (writes cup state to a throwaway temp path, reads it back, compares td5_save_get_cup_tier() before/after) -- never touches the real td5re_cup.ini. Camera/sound trace streams and net-loopback promotion remain todo (bigger, separate scope); drag natural-finish and the harness bug remain their own future package |
...
| G3 | td5_game.h migration + extern paydown | wip fix-refactor-g3-hud-externs-2 | SLICE 2 (in progress): pay down td5_hud.c's remaining 14 GLOBAL-VARIABLE externs → producer headers, taking hud.c 14→0. Mapped all 14: 3→td5_camera.h (g_subTickFraction/g_camWorldPos/g_cameraTransitionActive), 5→td5_game.h (g_special_render_mode/g_pending_finish_timer/g_race_end_state/g_actor_best_lap/g_actor_best_race), 2→td5_ai.h (g_cop_siren_warn_tick/g_wanted_hit_tick), 2 already-in-header pure deletes (g_wanted_damage_state@ai.h:42, g_render_width_f@render.h:358), 2 hud.c-local self-externs pure delete (g_pause_page_strings/sizes). Pure decl relocation; build+lint verifies, no launch. SLICE 1 DONE @abdfe319: 8 FUNCTION externs -- td5_ai_get_route_state (redundant, pure delete), td5_camera_apply_view/get_basis (added td5_camera.h include + deleted), 3x td5_game_* (added decls to td5_game.h) + 2x td5_track_* (added decls to td5_track.h). Pure declaration relocation, zero emitted-code change. extern_in_c_total 103->95 (hud.c 22->14); baseline JSON regenerated (also locks in S7's already-merged game_h_includers 25->24 + re-attributes S5/C9 split-file per-file counts, total unchanged by those). Build dev+release clean, no new warnings. Verified by build + structure-lint ratchet alone -- NO game launch (declaration-only change has no runtime surface; also a concurrent manual_drive session was holding the GPU). Left for follow-up slices: hud.c's 14 global-variable externs (g_*, no clean producer headers -- trickier), td5_game.c's 16 externs, and the broader td5_game.h->td5_race_state.h includer migration |
