...
| C2 | td5_camera.c cleanup | wip fix-refactor-c2-camera-gotos | SLICE 2 (in progress): 6 gotos -> structured control flow. Cluster A (after_flyin, 2 gotos): the goto only skips the fly-in-counter decrement -> invert the skip condition into each mode branch. Cluster B (store_prev, 4 gotos): each level branch skips the preset-reload block when prev==new preset -> a do_load flag guarding LoadCameraPresetForView. Behavior-preserving; camera is golden-guarded (fly-in/transition runs in the countdown, feeds the camera trace) so the 7-module goldens verify. PRIOR: FP migration @d515bf70. Preset-offset naming still after
