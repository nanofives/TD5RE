...
| C1 | Coverage build-out (camera/sound traces, save/net tests) | done (partial) @d5adf821 | save/load round-trip selftest step SHIPPED: new save-load-roundtrip verdict row in td5_selftest.c's report phase, wiring the dev-only explicit-path parameter td5_save_write_cup_data/load_cup_data were designed for. Writes cup statics to throwaway log/selftest_cup_roundtrip.ini (never the real td5re_cup.ini), reads back, asserts write/read + td5_save_is_cup_valid() both sides. GOTCHA handled: menu-phase game_type==0 hits cfgini's BY-DESIGN GameType<=0 empty-cup early-out on both sides (verifying nothing) -- the check temporarily sets game_type=CHAMPIONSHIP + syncs cup statics, restores after. Reconciled with the concurrent render-golden selftest layer (one include conflict). Verified: full suite 49/49 PASS (both trace-golden hash sets + both render-goldens matched), new row PASS write=1 valid=1 read=1 valid=1. Verification was blocked ~3h by a concurrent GPU-holding manual_drive session (multi-instance TDR hazard) -- succeeded first try once the GPU freed. REMAINING C1 scope (separate slices): camera/sound trace streams + goldens, net-loopback promotion into the suite; drag natural-finish + the escalated golden-harness order-sensitivity bug stay their own future package |
