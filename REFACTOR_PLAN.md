...
| C2 | td5_camera.c cleanup | wip fix-refactor-c2-camera-fp | FIRST SLICE (in progress): FP_* macro migration of td5_camera.c's 30 genuine 24.8 fixed-point shift sites (8 <<8 world/vertex-scale -> FP_SCALE, 22 >>8 truncate -> FP_TRUNC; classified per-site, comments + non-FP excluded). Now golden-guarded via the camera trace stream just shipped (@803883c4), so the 7-module goldens are the bit-identical verification net. Adds td5_fp.h include. Gotos(6)->structured + preset-offset naming left for follow-up slices
