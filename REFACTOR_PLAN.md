...
| C1 | Coverage build-out (camera/sound traces, save/net tests) | done (partial) @803883c4 | ALL THREE COVERAGE SUB-ITEMS SHIPPED: (1) save/load round-trip @d5adf821, (2) net-loopback promotion @789b9f0b, (3) camera+sound trace streams @803883c4 -- goldens now hash 7 modules (was 5). Camera stream = per-TICK solved pose via new td5_camera_get_tick_pose (NOT g_camWorldPos, whose subtick extrapolation is render-paced/nondeterministic -- why VIEW was never golden-hashed); sound stream = sim-event siren subset (frame-paced mixer state deliberately excluded). Goldens re-recorded: diff = exactly 4 new lines, all 10 existing hashes byte-identical; verified 50/50 PASS 0 WARN 0 FAIL clean run, both scenarios '7 module hashes match goldens'. C2/C7/C8's prereqs are now in place. REMAINING from the original C1 scope: only the escalated drag-natural-finish + golden-harness order-sensitivity investigation (its own future package) |
...
| C2 | td5_camera.c cleanup | done (partial) @d515bf70 | FIRST SLICE SHIPPED: FP_* macro migration of td5_camera.c's 30 genuine 24.8 fixed-point shifts (8 <<8->FP_SCALE, 22 >>8->FP_TRUNC; per-site classified, comments/non-FP excluded -- none of the latter here). Bit-identical (incl multi-line orient sin/cos truncates, unsigned-cast angle truncates, rb*0x102Cu>>8u). Golden-guarded via C1's camera stream: both scenarios '7 module hashes match goldens'. Build clean. (Run's frame-time FAIL = documented nvwgf2um.dll GPU/CPU-contention env signature, sim-independent.) REMAINING C2: 6 gotos -> structured, preset-offset naming -- follow-up slices
