...
| S1b | FP_* migration td5_physics_suspension.c | wip fix-refactor-s1b-fp-suspension | sim-exclusive; code done in worktree, BLOCKED on verification — re/assets got wiped repo-wide mid-session (unrelated to this package), recovery restored gameplay but golden-trace hashes now mismatch on already-shipped master too (proven not a code regression: reran selftest on a clean, unmodified master checkout, same wrong hashes). No sim package can be golden-verified until this is root-caused; do not merge S1b (or any sim package) until resolved. See feedback_golden_trace_baseline_broken_2026-07-10 memory |
