# [R11 GUARD] CHASE-cam frame at one span, for the guardrail-orientation A/B.
# Top-down flattens upright geometry, so item 11 ("guardrails are upside down")
# can only be judged from a camera that sees the rail face -- the chase cam.
# Matched pose: both sides park at the same StartSpanOffset on the same pinned
# seed and dump at the same elapsed race time.
param(
...
# PowerShell session, so an "isolated" A/B otherwise measures the union of
# every knob the session ever set.
...
$env:TD5RE_WINDOW_TITLE   = "R11GUARD-$Tag"
...
$p.Refresh()
if ($p.MainWindowHandle -ne 0) { [W3]::SetWindowPos($p.MainWindowHandle,[IntPtr]1,0,0,0,0,0x0013) | Out-Null }

$deadline = (Get-Date).AddSeconds($Wait)
while ((Get-Date) -lt $deadline -and -not $p.HasExited) { Start-Sleep -Milliseconds 500 }
if (-not $p.HasExited) {
    $p.Refresh()
    if ($p.MainWindowHandle -ne 0) { [W3]::PostMessage($p.MainWindowHandle,0x0010,[IntPtr]::Zero,[IntPtr]::Zero) | Out-Null }
