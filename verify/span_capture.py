...
# GENERATION FINGERPRINT -- READ THE CAVEAT.
# These counters are accumulated while scenery STREAMS during the drive, and the
# streaming loop breaks on cancel between entries, so a run that quits earlier
# publishes fewer entries and reports SMALLER numbers. They measure how far the
# car got, not what the generator produced. Do NOT use them to decide whether two
# runs generated the same track -- that mistake was made on 2026-09-04 and briefly
# recorded as a generator defect.
# GENERATION IS DETERMINISTIC. Verified 2026-09-04 the way the generator's own
# comment prescribes: three runs with TD5RE_AUTOTRACK_STREAM=0 (full build up
# front, no streaming) produced a byte-identical re/assets/levels/level090/
# MODELS.DAT, sha256 769ed15e36213c99... all three times. So an A/B pair with the
# same seed and knobs IS comparable; the frames differ only by capture timing.
# If you ever need to re-verify determinism, use that method (STREAM=0 + sha256 of
# MODELS.DAT), never these counters.
fp=[]
for pat in (r"spans=(\d+) len=(\d+)", r"paved_span_sides=(\d+)", r"outer_faces=(\d+)"):
    m=re.search(pat, log); fp.append(m.group(0) if m else "?")
print("GENFP(stream-progress, NOT a determinism check):", " | ".join(fp))
