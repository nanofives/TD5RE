/**
 * d3d11_backend_device.c - D3D11 device lifecycle, window management,
 * resource creation
 *
 ...
 *   - Photo-booth frame capture (offline car-preview generation)
 ...
 *   - Dynamic vertex/index buffer management + streaming upload
 *   - Multithreaded pane-record pool (deferred contexts)
 *   - Render target creation/release
 *   - Device creation, reset, exclusive-fullscreen toggle
 *
 * Split out of d3d11_backend.c (2026-07-09, A3 refactor) along with
 * d3d11_backend_pipeline.c (per-draw state/lighting passes) and
...
/* [diag] capture-path counters: serviced calls, and where they bail. Exposed via
 * Backend_CaptureDebug so the self-test render-golden layer can log why a frame
 * failed to read back. */
static unsigned s_cap_serviced, s_cap_getbuf_fail, s_cap_tex_fail,
                s_cap_map_fail, s_cap_ok, s_cap_no_swapchain;
static unsigned s_cap_last_hr, s_cap_fmt, s_cap_samples, s_cap_w, s_cap_h,
                s_cap_bind, s_cap_misc;
void Backend_CaptureDebug(unsigned *out)
{
    out[0]  = s_cap_serviced;   out[1]  = s_cap_no_swapchain;
    out[2]  = s_cap_getbuf_fail; out[3] = s_cap_tex_fail;
    out[4]  = s_cap_map_fail;   out[5]  = s_cap_ok;
    out[6]  = s_cap_last_hr;     out[7]  = s_cap_fmt;
    out[8]  = s_cap_samples;     out[9]  = s_cap_w;
    out[10] = s_cap_h;           out[11] = s_cap_bind;
    out[12] = s_cap_misc;
}
static void backend_do_capture(void)
{
    ...
    HRESULT hr;
    if (!g_backend.swap_chain) { s_cap_no_swapchain++; return; }
    hr = IDXGISwapChain_GetBuffer(g_backend.swap_chain, 0,
                                  &IID_ID3D11Texture2D, (void **)&bb);
    if (FAILED(hr) || !bb) { s_cap_getbuf_fail++; return; }
    D3D11_TEXTURE2D_DESC desc;
    ID3D11Texture2D_GetDesc(bb, &desc);
    s_cap_fmt = (unsigned)desc.Format;
    s_cap_samples = desc.SampleDesc.Count;
    s_cap_w = desc.Width; s_cap_h = desc.Height;
    s_cap_bind = desc.BindFlags; s_cap_misc = desc.MiscFlags;
    ...
    if (FAILED(hr)) { s_cap_last_hr = (unsigned)hr; s_cap_tex_fail++;
                      Backend_NoteDeviceRemoved(hr, "capture/CreateTexture2D");
    ...
    if (SUCCEEDED(hr)) {
        ...
        s_cap_map_fail++;
        s_cap_last_hr = (unsigned)hr;
        Backend_NoteDeviceRemoved(hr, "capture/Map");
    }
    ID3D11Texture2D_Release(st);
    ID3D11Texture2D_Release(bb);
}
/* [DEVICE-LOST 2026-07-13] Detect + latch a removed GPU device, and log WHY.
 * A TDR (timeout) or a driver fault removes the device; if we keep issuing
 * commands on it the driver eventually NULL-derefs and crashes the process
 * (0xC0000005). This converts that into a clean, logged, survivable state. */
int Backend_NoteDeviceRemoved(HRESULT hr, const char *where)
{
    if (g_backend.device_removed) return 1;   /* already latched */
    /* Only the device-lost family of HRESULTs; healthy calls fall through. */
    ...
    HRESULT reason = g_backend.device
                     ? ID3D11Device_GetDeviceRemovedReason(g_backend.device)
                     : (HRESULT)0;
    const char *rn;
    switch ((unsigned)reason) {
        case 0x887A0006u: rn = "DEVICE_HUNG (a draw/dispatch ran too long -> TDR)"; break;
        case 0x887A0005u: rn = "DEVICE_REMOVED (physical removal / driver update / TDR)"; break;
        case 0x887A0007u: rn = "DEVICE_RESET (a draw caused a device reset)"; break;
        case 0x887A0020u: rn = "DRIVER_INTERNAL_ERROR (driver bug / bad state)"; break;
        case 0x887A0001u: rn = "INVALID_CALL (WE issued an illegal D3D call)"; break;
        case 0x00000000u: rn = "S_OK (device not actually removed?!)"; break;
...
{
    if (g_backend.device_removed || !s_capture_request) return;
    s_cap_serviced++; backend_do_capture(); s_capture_request = 0;
}
...
/* [lighting rework P3] SS reflections pass */
#include "shaders/ps_ssr_bytes.h"
...
static int Backend_CreateShaders(void)
{
    ...
    /* [P3] SS reflections pass (non-fatal: NULL => SSR pass is skipped). */
    hr = ID3D11Device_CreatePixelShader(dev,
        g_ps_ssr, sizeof(g_ps_ssr), NULL,
        &g_backend.ps_ssr);
    if (FAILED(hr)) { WRAPPER_LOG("CreateShaders: ps_ssr FAILED (SSR pass disabled)"); g_backend.ps_ssr = NULL; }
...
static int Backend_CreateDynamicBuffers(void)
{
    ...
    /* [P3] SSR pass constant buffer (non-fatal). */
    bd.ByteWidth = sizeof(SSRCB);
    hr = ID3D11Device_CreateBuffer(g_backend.device, &bd, NULL, &g_backend.cb_ssr);
    if (FAILED(hr)) { WRAPPER_LOG("CreateDynamicBuffers: cb_ssr FAILED (SSR pass disabled)"); g_backend.cb_ssr = NULL; }
...
void Backend_RecPoolRelease(void)
{
    for (int i = 0; i < s_rec_pool_count; i++) {
        ...
        rc->current_srv = NULL;   /* freed with the device; don't leave a stale bind */
    }
    s_rec_pool_count = 0;
}
...
static void Backend_ReleaseRenderTargets(void)
{
    ...
    /* [P3] Scene-colour copy tracks the backbuffer surface size — drop it too
     * (the next SSR pass recreates it to match). */
...
/* Backend teardown of every D3D11/DXGI object (moved here from ddraw_main.c so
 * that shared COM file holds no ID3D11 types). */
