...
void td5_sound_update_vehicle_looping_state(int actor_index)
{
    ...
    if (td5_game_is_wanted_mode() && actor_index == td5_game_get_cop_actor_index()) {
         ...
         * (fade the siren out) and the mixer then physically stops the siren channels
         * + clears s_tracked_veh_active. If we kept refreshing here, s_siren_active_flag
         * would stay latched at 1 every paused frame, so on resume the
         ...
         * Mirrors the pause gating already applied to td5_sound_update_police_siren. */
