...
static const TD5_ChangelogLine k_changelog_lines[] = {
    ...
    { CL_ITEM,    "AI MODEL is hidden when there are no opponents (time" },
    { CL_ITEM,    "  trial, or OPPONENTS set to 0)." },
    { CL_ITEM,    "Time Trial: the AUTO/MANUAL gearbox choice now takes" },
    { CL_ITEM,    "  effect in the race." },
    { CL_ITEM,    "Drag race: gearbox is locked to MANUAL and the" },
    { CL_ITEM,    "  selector is greyed out." },
    { CL_ITEM,    "View Race Data no longer flashes the table before it" },
    { CL_ITEM,    "  slides in." },
    { CL_BLANK,   "" },
    ...
    { CL_ITEM,    "Water no longer sheets over the land or the road near" },
    { CL_ITEM,    "  a distant river. A river's plane now stops at its far" },
    { CL_ITEM,    "  bank instead of running a fixed sheet across the map," },
    { CL_ITEM,    "  which is what put water over the road where it looped." },
    ...
    { CL_ITEM,    "  and ends. The finish dot used to sit past the line by" },
    { CL_ITEM,    "  the whole RUN-OFF setting, up to 400 spans." },
    { CL_ITEM,    "AUTO TRACK STUDIO: typing a seed now shows the digits" },
    { CL_ITEM,    "  as you type them. The row was already editable but" },
    { CL_ITEM,    "  always drew the saved value, so it looked dead." },
    { CL_ITEM,    "Guardrails, sidewalks, scenery, bridges, tunnels and" },
    { CL_ITEM,    "  18 more rows now REALLY vary by seed. They showed a" },
    { CL_ITEM,    "  '~' but were weighted to one value and never moved." },
    { CL_ITEM,    "  A budget stops a seed dropping too many at once." },
    { CL_ITEM,    "No more grass inside tunnels, and the landscape now" },
    { CL_ITEM,    "  runs OVER a tunnel instead of leaving a slot cut" },
    { CL_ITEM,    "  through it: ground over a bore follows the real hill," },
    ...
    { CL_ITEM,    "  and plains. The road is laid onto it and steers" },
    { CL_ITEM,    "  around water and slopes over 30 degrees." },
    { CL_ITEM,    "Bridges and tunnels are DETECTED from the terrain, not" },
    { CL_ITEM,    "  placed by a dice roll: water is always bridged, a" },
    { CL_ITEM,    "  mountain in the way is bored, and both are capped in" },
    { CL_ITEM,    "  length (the planner asks the map where they exit)." },
    { CL_ITEM,    "Side streets form a real network: they join each other," },
    { CL_ITEM,    "  close blocks with back streets, and country lanes" },
    { CL_ITEM,    "  loop back to the main road. None of them cross." },
    { CL_ITEM,    "Long forks become BYPASS roads planned by the network:" },
    { CL_ITEM,    "  drivable corridors shaped by the terrain around them." },
    { CL_ITEM,    "Terrain, water and buildings sample the world, so a" },
    { CL_ITEM,    "  viaduct shows the valley under it and coasts read" },
    { CL_ITEM,    "  as coasts. Old seeds produce different tracks now." },
    { CL_BLANK,   "" },

    { CL_DATE,    "September 08 (sound options)" },
    { CL_ITEM,    "SOUND OPTIONS is now SFX and RADIO volume. The music" },
    { CL_ITEM,    "  slider moved out; the pause menu still has it." },
    { CL_ITEM,    "New RADIO VOLUME row sets the internet-radio level" },
    { CL_ITEM,    "  from the menu instead of only the pause screen." },
    { CL_ITEM,    "New RADIO STATION row: pick your own station. It" },
    { CL_ITEM,    "  shows the address format and rejects a bad one." },
    { CL_ITEM,    "While the radio volume is not zero, the screen lists" },
    { CL_ITEM,    "  the station, its address and the connection state." },
    { CL_ITEM,    "The OK button no longer shows left/right arrows it" },
    { CL_ITEM,    "  could never do anything with." },
    { CL_BLANK,   "" },
    ...
    { CL_ITEM,    "Distant scenery no longer draws across the road on a" },
    { CL_ITEM,    "  hairpin, and two backdrops no longer overlap." },
    { CL_ITEM,    "A city skyline no longer appears behind a coast." },
    { CL_ITEM,    "Roadside trees on a branch sit on the ground, and" },
    { CL_ITEM,    "  nothing stands over a river any more." },
    { CL_ITEM,    "Grass stops spilling onto the road on a sharp bend." },
    { CL_ITEM,    "Pavement carries through a junction on the built" },
    { CL_ITEM,    "  side, so frontages keep their footway." },
    { CL_ITEM,    "The sea no longer cuts off where a side road clears." },
    { CL_ITEM,    "A bridge deck reads as one surface, with no sunken" },
    { CL_ITEM,    "  strip starting mid-crossing." },
    { CL_BLANK,   "" },

    { CL_DATE,    "September 07 (generated tracks, later)" },
    { CL_ITEM,    "Guardrails read as ONE rail. Two of the four rail" },
    { CL_ITEM,    "  textures are two-beam fences, which looked stacked." },
    { CL_ITEM,    "Lane markings stay continuous through a fork: they" },
    { CL_ITEM,    "  used to double up and jump where a road splits." },
    { CL_ITEM,    "A planted median now reaches the lane edge, so no" },
    { CL_ITEM,    "  strip of grass is left lying on the road." },
    { CL_ITEM,    "The forest backdrop no longer stands in the sea, and" },
    { CL_ITEM,    "  no longer runs across the opposite carriageway." },
    { CL_ITEM,    "Ground and trees are continuous through a biome" },
    { CL_ITEM,    "  change instead of appearing every other span." },
    { CL_ITEM,    "Park lawns, kerbside blocks and fork greenery sit on" },
    { CL_ITEM,    "  the ground instead of floating past the verge." },
    { CL_ITEM,    "A bridge bank drops to the water at the run ends" },
    { CL_ITEM,    "  instead of leaving grass hanging over the river." },
    { CL_BLANK,   "" },

    { CL_DATE,    "September 07 (generated tracks)" },
    { CL_ITEM,    "Tunnels in forest keep ONE facade. The stacked rock and" },
    { CL_ITEM,    "  masonry boxes are gone and the tree line wraps the" },
    { CL_ITEM,    "  mouth; the gap at the entrance is welded shut." },
    { CL_ITEM,    "Tunnels have a narrow inner walkway with railings." },
    { CL_ITEM,    "Bridges no longer stand two pillars side by side at" },
    { CL_ITEM,    "  the centre tower." },
    { CL_ITEM,    "A bridge bank now drops steeply to the water instead" },
    { CL_ITEM,    "  of sloping back into the road." },
    { CL_ITEM,    "Town sidewalks join up through side-street mouths on" },
    { CL_ITEM,    "  the edge of town, with the crossing markings." },
    { CL_ITEM,    "Retracted town blocks are dressed as small parks." },
    { CL_ITEM,    "Buildings too small to draw properly are dropped." },
    { CL_ITEM,    "Tiny stub medians are gone; the ones that remain" },
    { CL_ITEM,    "  stand taller and run longer." },
    ...
    { CL_DATE,    "September 06 (generated tracks)" },
    { CL_ITEM,    "Stop signs now stand on a post, and only at junctions" },
    { CL_ITEM,    "  instead of along open frontage." },
    { CL_ITEM,    "Shop fronts no longer repeat the same signage on two" },
    { CL_ITEM,    "  neighbouring buildings." },
    { CL_ITEM,    "The distant city skyline is no longer smeared: its" },
    { CL_ITEM,    "  texture tiles at a constant size through bends." },
    { CL_ITEM,    "Low buildings are closed at the back, so they no longer" },
    { CL_ITEM,    "  look hollow from a side street or a rise." },
    ...
    { CL_ITEM,    "AUTO TRACK STUDIO: six fork shapes, not one. Divided" },
    ...
    { CL_ITEM,    "AUTO TRACK STUDIO: real lane management. Roads gain and" },
    { CL_ITEM,    "  lose lanes along the way (2 to 8) the way the shipped" },
    { CL_ITEM,    "  tracks do: one-sided drops and merges, symmetric" },
    { CL_ITEM,    "  widenings, lane paint and AI following the count." },
    { CL_ITEM,    "  TD5RE_AUTOTRACK_LANE_VARY=0 restores the fixed 4 lanes." },
    ...
    { CL_ITEM,    "  root publish copy, which the game never reads. 10 rows" },
    { CL_ITEM,    "  retired by content match: 7 arcade/cop rows MERGED into" },
    { CL_ITEM,    "  their successors, 2 SUPERSEDED box-placement rows, 1" },
    { CL_ITEM,    "  selftest row as INFO (verified by script, not driving)." },
    { CL_ITEM,    "  List drops 320 -> 310. The net cop-chase row was kept:" },
    { CL_ITEM,    "  master holds one copy, so 'duplicate' would hide it." },
    { CL_ITEM,    "FIX: the PENDING TO TEST screen showed only 58 items." },
    { CL_ITEM,    "  The list loaded into a fixed 256 KB buffer and the CSV" },
    { CL_ITEM,    "  passed that on Aug 25, so rows past the first ~58 were" },
    ...
    { CL_ITEM,    "FIX: generated tracks reach the green light in about a" },
    { CL_ITEM,    "  third of the time, and the game no longer freezes while" },
    { CL_ITEM,    "  one is built. Launch to race was 76 s: the generator ran" },
    { CL_ITEM,    "  TWICE per launch (a throwaway build at boot, 20 s, then" },
    { CL_ITEM,    "  the real one), two diagnostic sweeps meant as opt-in ran" },
    { CL_ITEM,    "  on every build (10 s), and all of it blocked the window." },
    { CL_ITEM,    "  Now: no boot build; the race build runs on a worker" },
    { CL_ITEM,    "  thread under the loading screen with a progress bar;" },
    { CL_ITEM,    "  the sweeps are off unless asked for; the street-crossing" },
    { CL_ITEM,    "  test that every emitter re-asked 48 spans back is cached." },
    { CL_ITEM,    "  Same seed, same machine: 34 s -> 14 s to build, 76 s ->" },
    ...
    { CL_ITEM,    "FIX: ray-traced lighting on generated tracks. Two things" },
    { CL_ITEM,    "  were wrong. The ray-traced scene held only the first" },
    { CL_ITEM,    "  1900 scenery meshes of a track; a generated track has" },
    { CL_ITEM,    "  about 14000, so past the first 240 spans nothing cast" },
    { CL_ITEM,    "  a shadow or showed in a reflection. And every one of" },
    { CL_ITEM,    "  those meshes was its own tiny instance, which is what" },
    { CL_ITEM,    "  the rays were paying for: on a tree-lined start the" },
    { CL_ITEM,    "  frame rate at 2560x1351 was 82-93, against 165 on" },
    { CL_ITEM,    "  Moscow with the same settings." },
    { CL_ITEM,    "  Now the scene is a window around the car (400 spans" },
    { CL_ITEM,    "  ahead, 120 behind, per split-screen pane) streamed in" },
    { CL_ITEM,    "  and out as you drive, and each stretch of track is one" },
    { CL_ITEM,    "  merged structure instead of dozens of separate ones." },
    { CL_ITEM,    "  Same view, same settings: 258 -> 534 fps at the start" },
    { CL_ITEM,    "  line, 82-93 -> ~200 driving the forest start; the town" },
    { CL_ITEM,    "  at span 500 now carries 4300 meshes at ~130." },
    { CL_ITEM,    "  Sky-visibility (GI) rays treat tree canopies and fences" },
    ...
    { CL_ITEM,    "  built in about half a second and the race starts;" },
    { CL_ITEM,    "  the buildings, trees and bridges are then filled in" },
    { CL_ITEM,    "  on a background thread while you drive, faster than" },
    { CL_ITEM,    "  you can reach them." },
    { CL_ITEM,    "  Same seed, same machine: building a track was 20 s" },
    { CL_ITEM,    "  and is now 0.5 s, and the wait from launching to the" },
    { CL_ITEM,    "  green light drops from 38 s to 12 s. The finished" },
    { CL_ITEM,    "  track is identical, to the byte." },
    { CL_ITEM,    "  The countdown holds briefly so the road ahead of the" },
    { CL_ITEM,    "  grid is decorated before the lights go green. If the" },
    { CL_ITEM,    "  scenery ever falls behind you, the road ahead shows" },
    { CL_ITEM,    "  as plain grey tarmac rather than disappearing." },
    { CL_ITEM,    "  Set TD5RE_AUTOTRACK_STREAM=0 for the old behaviour." },
    { CL_BLANK,   "" },
    { CL_BLANK,   "" },
    ...
    { CL_ITEM,    "FIX: generated tracks load about three times faster." },
    { CL_ITEM,    "  Building one blocked the game for around 92 seconds," },
    { CL_ITEM,    "  with the window greyed out as Not Responding for the" },
    { CL_ITEM,    "  whole wait -- long enough to read as a crash. Now 29." },
    { CL_ITEM,    "  Two passes were repeating work that could not change:" },
    { CL_ITEM,    "  the exposed-rear test rebuilt the same building it" },
    { CL_ITEM,    "  was measuring once per camera (81 times), and the" },
    { CL_ITEM,    "  dry-band test rescanned 91 spans for river at each of" },
    { CL_ITEM,    "  its 41 samples. Both now compute it once and reuse it." },
    { CL_ITEM,    "  Pure speed: MODELS.DAT and STRIP.DAT are byte-for-byte" },
    { CL_ITEM,    "  what they were on seeds 20260901 and 20260902, so the" },
    { CL_ITEM,    "  track you get is unchanged, it just arrives sooner." },
    { CL_ITEM,    "  TD5RE_R14_GENPROF=0 silences the new timing report." },
    { CL_BLANK,   "" },

    { CL_DATE,    "September 01 (auto-track forest crossings, r14)" },
    { CL_ITEM,    "FIX: nothing stands in a forest side road any more." },
    { CL_ITEM,    "  A crate and a pair of deer were sitting in the" },
    { CL_ITEM,    "  crossing lane: roadside furniture and animals set" },
    { CL_ITEM,    "  their distance from the MAIN road edge, and neither" },
    { CL_ITEM,    "  knew a forest lane had been laid there. Furniture" },
    { CL_ITEM,    "  and billboards are now refused over the lane, an" },
    { CL_ITEM,    "  animal moves to the other verge instead (so the" },
    { CL_ITEM,    "  count is unchanged), and trees are kept off one" },
    { CL_ITEM,    "  span further out because a trunk on the shoulder" },
    { CL_ITEM,    "  still leaned over the mouth. Seed 20260901: 4" },
    { CL_ITEM,    "  pieces refused, 2 animals moved; seed 20260902: 3" },
    { CL_ITEM,    "  refused. TD5RE_R14_FCROSS_CLEAR=0 reverts." },
    { CL_ITEM,    "FIX: the verge pavement now STOPS at a forest side" },
    { CL_ITEM,    "  road instead of being painted across its throat." },
    { CL_ITEM,    "  6 span-sides ended at a mouth on both test seeds." },
    { CL_ITEM,    "  TD5RE_R14_FCROSS_PAVESTOP=0 reverts." },
    { CL_ITEM,    "NEW: most forest side roads are now a worn, rutted" },
    { CL_ITEM,    "  dirt lane rather than city asphalt with a painted" },
    { CL_ITEM,    "  centre line. Rate went 0% to 66% on seed 20260901" },
    { CL_ITEM,    "  and 33% on 20260902. TD5RE_R14_FCROSS_WORN=0" },
    { CL_ITEM,    "  restores the marked surface on every crossing." },
    { CL_ITEM,    "FIX: a forest side road is held to a full two lanes." },
    { CL_ITEM,    "  Its width is the chord between two road nodes, so" },
    { CL_ITEM,    "  a bend made the mouth narrower than the two spans" },
    { CL_ITEM,    "  it is built from. The width is measured and padded" },
    { CL_ITEM,    "  back up to 3000 units, never past it. Both test" },
    { CL_ITEM,    "  seeds already measured 2996-2999, so the pad is" },
    { CL_ITEM,    "  0-1 there. TD5RE_R14_FCROSS_WIDE=0 reverts." },
    { CL_ITEM,    "FIX: no more doubled pavement where a branch starts." },
    { CL_ITEM,    "  At the split the corridor's own kerb begins on the" },
    { CL_ITEM,    "  exact lateral the main road's kerb already occupies," },
    { CL_ITEM,    "  so 1011 units of pavement were laid twice, one slab" },
    { CL_ITEM,    "  inside the other. The corridor now OWNS that edge" },
    { CL_ITEM,    "  and the main slab hands it over instead of tapering." },
    { CL_ITEM,    "FIX: no more one-span holes in a fork's pavement." },
    { CL_ITEM,    "  The kerb was stopping for a side-street mouth that" },
    { CL_ITEM,    "  the fork guard had already removed, so it broke for" },
    { CL_ITEM,    "  a street nothing opens. 11 holes on seed 20260901," },
    { CL_ITEM,    "  three of them one span wide -- now none." },
    { CL_ITEM,    "FIX: the pavement's outer face now has height." },
    { CL_ITEM,    "  The slab carried a top and a road-facing kerb and" },
    { CL_ITEM,    "  nothing closing its back edge, so from behind it" },
    { CL_ITEM,    "  read as zero-thickness. Both pavement emitters now" },
    { CL_ITEM,    "  close it down into the ground." },
    { CL_ITEM,    "FIX: an overpass no longer stands over a void." },
    { CL_ITEM,    "  The deck already reached as far as the drawn ground" },
    { CL_ITEM,    "  did, so the floor was not short -- it was SINKING." },
    { CL_ITEM,    "  Past the 12000 verge the far band dives toward the" },
    { CL_ITEM,    "  track floor while the deck stays flat, so measured" },
    { CL_ITEM,    "  soffit-to-ground ran 2600 to 33795 across one arm." },
    { CL_ITEM,    "  The skirt now stays flat out to the deck's own reach" },
    { CL_ITEM,    "  over the crossing, tapered back over 4 spans. Costs" },
    { CL_ITEM,    "  no new meshes and keeps the road and fold clamps." },
    { CL_ITEM,    "FIX: the town closes up around a crossing." },
    { CL_ITEM,    "  The no-massing corridor under the deck stays (a" },
    { CL_ITEM,    "  facade there would intersect it), but the spans just" },
    { CL_ITEM,    "  outside it now take the r13 infill block wherever" },
    { CL_ITEM,    "  nothing stands -- no corner-distance rule and no" },
    ...
    { CL_ITEM,    "FIX: the shore beside a bridge no longer cuts through" },
    { CL_ITEM,    "  the ground. The coastline band reaches 33000 units" },
    { CL_ITEM,    "  out but took its height from the near verge, which" },
    { CL_ITEM,    "  is only defined to 12000, then held that height" },
    { CL_ITEM,    "  flat for the rest -- so the verge sat 1270 to 3944" },
    { CL_ITEM,    "  units above the band, and where the land fell away" },
    { CL_ITEM,    "  the rim left a step of up to 26179. It now reads" },
    { CL_ITEM,    "  the whole-width ground chain the far band is built" },
    { CL_ITEM,    "  from, so the shore lands on the terrain: 0 on both" },
    { CL_ITEM,    "  samples at every bridge end, on two seeds." },
    { CL_ITEM,    "FIX: the tree line now WRAPS AROUND water instead of" },
    { CL_ITEM,    "  stopping on a flat face. The band deleted itself" },
    { CL_ITEM,    "  over a whole bridge run, so a 14000-tall wall" },
    { CL_ITEM,    "  ended dead at one mouth and reappeared at full" },
    { CL_ITEM,    "  height at the other -- 10 of 17 wall ends on seed" },
    { CL_ITEM,    "  20260901. At each mouth the wall now turns and" },
    { CL_ITEM,    "  runs outward along the shore, tapering to nothing" },
    { CL_ITEM,    "  where the ground meets the water. Adds geometry" },
    { CL_ITEM,    "  only: band presence is untouched, so the r13 far" },
    { CL_ITEM,    "  band cull decides exactly what it did before." },
