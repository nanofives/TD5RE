/*
 * td5_pilot_trace_00404030.c -- Port-side CSV emitter for
 * UpdatePlayerVehicleDynamics pilot (pool0 / 0x00404030).
 *
 * Mirrors tools/frida_pool0_00404030.js column-for-column.
 *
 * Slot filter: target slot 0 only.
 */
#include "td5_pilot_trace_00404030.h"
...
#define OUT_PATH "log/port/pool0_00404030.csv"

/* Actor field offsets (mirrored from td5_types.h with byte addresses) */
...
#define O_ENGINE_SPEED_ACC  0x33A
...
#define O_HANDBRAKE_FLAG    0x36E
#define O_SURFACE_CHASSIS   0x370
#define O_SURFACE_CONTACT   0x376
...
#define C_DT_LAYOUT    0x76
#define C_HBRAKE_FACT  0x7A
#define C_SLIP_COUPL   0x7C
...
typedef struct {
    ...
    uint8_t  surface_contact_in;
    int16_t  track_span_raw;
    uint8_t  sub_lane_index;
    int32_t  euler_yaw_in;
    int32_t  lin_vx_in, lin_vz_in;
    int32_t  ang_yaw_in;
    int32_t  steer_cmd_in;
    int16_t  encounter_steer_in;
    uint8_t  gear_in, brake_flag_in, handbrake_flag_in;
    int16_t  engine_speed_acc_in;
    int32_t  center_pos_in, center_vel_in;
    int32_t  susp_pos_in[4];
    int32_t  long_speed_in, lat_speed_in;
    int32_t  inertia, half_wb;
    int16_t  Wf, Wr, tire_grip;
    int16_t  damp_high, damp_low, brake_front, brake_rear;
    int16_t  speed_limit, dt_layout, hbrake_factor, slip_coupling;
} PilotInputs;
...
static void emit_header(FILE *fp) {
    fprintf(fp,
        ...
        "actor_addr,cardef_ptr,surface_chassis_in,surface_contact_in,"
        "track_span_raw,sub_lane_index,"
        "euler_yaw_in,lin_vx_in,lin_vz_in,ang_yaw_in,"
        "steer_cmd_in,encounter_steer_in,"
        "gear_in,brake_flag_in,handbrake_flag_in,"
        "engine_speed_acc_in,"
        "center_pos_in,center_vel_in,"
        "susp_pos0_in,susp_pos1_in,susp_pos2_in,susp_pos3_in,"
        "long_speed_in,lat_speed_in,"
        "inertia,half_wb,Wf,Wr,tire_grip,"
        "damp_high,damp_low,brake_front,brake_rear,"
        "speed_limit,dt_layout,hbrake_factor,slip_coupling,"
        "lin_vx_out,lin_vz_out,ang_yaw_out,"
        "long_speed_out,lat_speed_out,"
        "front_slip_excess_out,rear_slip_excess_out,slip_metric_out,"
        "tire_slip_x_out,tire_slip_z_out,"
        "surface_chassis_out,surface_contact_out,"
...
void td5_pilot_emit_00404030_enter(const TD5_Actor *actor, uintptr_t caller_ra) {
    ...
    s_inputs.surface_contact_in = rd_u8(base + O_SURFACE_CONTACT);
    ...
    s_inputs.handbrake_flag_in  = rd_u8(base + O_HANDBRAKE_FLAG);
    s_inputs.engine_speed_acc_in = rd_i16(base + O_ENGINE_SPEED_ACC);
    ...
    s_inputs.long_speed_in = rd_i32(base + O_LONG_SPEED);
    s_inputs.lat_speed_in  = rd_i32(base + O_LAT_SPEED);
    ...
    if (cardef) {
        ...
        s_inputs.dt_layout     = rd_i16(cardef + C_DT_LAYOUT);
        s_inputs.hbrake_factor = rd_i16(cardef + C_HBRAKE_FACT);
        s_inputs.slip_coupling = rd_i16(cardef + C_SLIP_COUPL);
    } else {
        s_inputs.inertia = s_inputs.half_wb = 0;
        s_inputs.Wf = s_inputs.Wr = s_inputs.tire_grip = 0;
        s_inputs.damp_high = s_inputs.damp_low = 0;
        s_inputs.brake_front = s_inputs.brake_rear = 0;
        s_inputs.speed_limit = s_inputs.dt_layout = 0;
        s_inputs.hbrake_factor = s_inputs.slip_coupling = 0;
    }
}
...
void td5_pilot_emit_00404030_leave(const TD5_Actor *actor) {
    ...
    uint8_t surface_contact_out = rd_u8(base + O_SURFACE_CONTACT);
    ...
    fprintf(s_fp,
        ...
        s_inputs.surface_chassis_in, s_inputs.surface_contact_in,
        ...
        s_inputs.steer_cmd_in, s_inputs.encounter_steer_in,
        s_inputs.gear_in, s_inputs.brake_flag_in, s_inputs.handbrake_flag_in,
        (int)s_inputs.engine_speed_acc_in,
        ...
        s_inputs.long_speed_in, s_inputs.lat_speed_in,
        s_inputs.inertia, s_inputs.half_wb,
        (int)s_inputs.Wf, (int)s_inputs.Wr, (int)s_inputs.tire_grip,
        (int)s_inputs.damp_high, (int)s_inputs.damp_low,
        (int)s_inputs.brake_front, (int)s_inputs.brake_rear,
        (int)s_inputs.speed_limit, (int)s_inputs.dt_layout,
        (int)s_inputs.hbrake_factor, (int)s_inputs.slip_coupling,
        lin_vx_out, lin_vz_out, ang_yaw_out,
        long_speed_out, lat_speed_out,
        front_slip_excess_out, rear_slip_excess_out, (int)slip_metric_out,
        (int)tire_slip_x_out, (int)tire_slip_z_out,
        surface_chassis_out, surface_contact_out,
        center_pos_out, center_vel_out,
