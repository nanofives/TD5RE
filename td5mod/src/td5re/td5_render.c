...
void td5_render_actors_for_view(int view_index)
{
    ...
    /* The original binary (0x42BB2E) limits span iteration based on the
     * view distance setting, but it walks spans outward from the player's
     * current position.  Our loop walks from span 0, so the setting would
     * cut off the tail of the array instead of limiting draw range.
     * For now, render all spans and rely on the per-mesh frustum cull. */
