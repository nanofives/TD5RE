...
typedef enum TD5_RenderPreset {
    ...
    TD5_PRESET_FOLIAGE_AA_FEATHER = 14  /* [foliage feather 2026-08-11] Identical D3D state to TRANSLUCENT_ANISO (SRCALPHA/INVSRCALPHA, aniso, MODULATEALPHA, z_write off) EXCEPT alpha_ref=0x81 instead of 0x80. The +1 is a wrapper-side MARKER: Backend_UpdateFogCB treats 0x80 and 0x81 identically for the foliage-AA gate, but sets FogCB.foliageFeather only for 0x81, so the pixel shader feathers (soft alpha ramp) the silhouette edge. Used ONLY for dark green TREE-canopy billboard pages (td5_asset_get_page_dark_foliage), so signs/banners (0x80) stay crisp. */
} TD5_RenderPreset;
