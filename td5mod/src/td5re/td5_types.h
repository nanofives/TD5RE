...
typedef enum TD5_RenderPreset {
    ...
    TD5_PRESET_TRANSLUCENT_POINT_ZTEST = 12, /* Same as TD5_PRESET_TRANSLUCENT_POINT (SRCALPHA/INVSRCALPHA, point filter, alpha_ref=1) but z_test=LEQUAL z_write=0 — depth-tested translucent billboard. Used by vehicle brake lights so a nearer car body occludes a farther car's brake light, matching the original's deferred z-tested tail-light flush (RenderVehicleTaillightQuads @0x4011C0 -> FlushQueuedTranslucentPrimitives @0x431340, ZENABLE on, ZFUNC=LESSEQUAL @0x40B070). No polygon_offset (NOT a ground decal). */
    TD5_PRESET_VEHICLE_FADE       = 13, /* [dynamic-traffic] Fading car body: SRCALPHA/INVSRCALPHA blend, LINEAR filter, MODULATEALPHA, z_test=LEQUAL with z_write ON (keeps self-occlusion sane while a whole mesh is semi-transparent), alpha_ref=1 (a fade alpha below 0x80 must NOT be alpha-tested away like TRANSLUCENT_ANISO's 0x80 cutoff would). The original has no per-actor fade (hard span cull only @ 0x0040C2FD); this preset exists solely for the port's GTA-style traffic fade in/out. */
    TD5_PRESET_FOLIAGE_CUTOUT     = 14, /* [foliage occlusion 2026-08-17] Billboard tree/foliage cutout drawn as ALPHA-TESTED OPAQUE with z-write ON, so it correctly occludes (and is occluded by) world geometry -- houses BEHIND a tree no longer paint over it. Blend OFF (no see-through canopy, no order dependence), z_enable/z_write ON, alpha_test ON, alpha_ref 0x80 (the discriminator the backend foliage-AA gate keys on, shared with TRANSLUCENT_ANISO), MODULATEALPHA, LINEAR filter. Replaces the earlier TRANSLUCENT_ANISO billboard promotion whose z-write-OFF alpha blend broke depth ordering. Edge cleanliness (dither merge + alpha-weighted RGB, no black fringe) is handled shader-side in SampleFoliageAA; gated by TD5RE_FOLIAGE_AA. */
    TD5_PRESET_WORLD_CUTOUT       = 15  /* [R15 items 10/11 2026-09-06] NON-billboard type-1 world cutout: a city railing, fence or sign built as ordinary quad geometry. Identical to OPAQUE_LINEAR except alpha_ref 0x01 -> 0x80. MEASURED CAUSE: OPAQUE_LINEAR keeps every LINEAR-interpolated texel whose alpha >= 1/255, and the balustrade page (k_furn_fence_idx, page 46) has a MEDIAN BALUSTER OF 2 TEXELS with 97% of runs <= 3 -- so a one-texel bilinear dilation lifts its coverage from 43% to 69%, the see-through gaps close, and the railing renders as a near-solid panel that OCCLUDES the road and the wall behind it. That is the reported "guardrail clipping over the main road as well as this building"; a validated per-quad SAT pass proved there is NO geometric interpenetration. Testing at 0x80 keeps a 2-texel bar 2 texels wide. Side effect BY DESIGN: 0x80 + (blend OFF, z-write ON) is exactly the backend's foliage-AA discriminator, so these draws also pick up SampleFoliageAA -- which the wrapper comment at d3d12_backend.c already names "foliage/FENCE/sign cutout draws" as its intended set; world fences simply never reached it because they carried alpha_ref 0x01. Gated by TD5RE_R15_WORLD_CUTOUT. */
} TD5_RenderPreset;
