/**
 * td5_frontend_button_cache.h -- Baked main-menu button surface cache
 *
 * Phase 6 of frontend mainmenu parity. Mirrors the original
 * CreateFrontendDisplayModeButton (0x00425DE0) lifecycle:
 *   - 224x64 BGRA texture per button.
 *   - Top half  (y = 0..h-1)  = state 1 (unselected, transparent interior).
 *   - Bottom half (y = h..2h-1) = state 0 (selected,  purple interior).
 *
...
#define TD5_FE_BTNCACHE_MAX 8  /* Main menu has 7 buttons; one slack slot */
...
/* Returns the absolute texture page for `index` if the cached label matches,
 * or -1 if not cached / label mismatch. The label match guards against the
 * cache being reused for a different button after a screen rebuild. */
 ...
 * frontend_recover_surfaces() so cached buttons survive device reset / native
 * resolution changes. No-op for empty slots. */
