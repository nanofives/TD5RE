...
| 4 | DXPSTART | 4 B + `TD5_NetRaceConfig` (S31: rng_seed, track, direction, laps, per-slot car/paint) | Host→each client at race launch (lobby state 0x10); both sides archive the config (`td5_net_get_race_config`). Client auto-replies ONE type 6. |
...
- **Race-config replication (S31, 2026-06-10)**: the host builds a `TD5_NetRaceConfig` at state 0x10 (seed, `s_selected_track`, direction, per-slot cars from the CAR_INFO announcements) and broadcasts it in the DXPSTART payload. Clients adopt track/direction before running their schedule; the schedule fills net-player slots from the config and re-seeds the CRT with the shared seed so the rand()-driven AI fill picks identical cars everywhere. The legacy DXPDATA config-push states 0xC-0xF remain vestigial. `Screen_SessionLocked` [29] still shows only on `s_kicked_flag`, which **no remote peer ever sets** (kick wire = follow-up).
...
**RNG seed (S31, 2026-06-10): exchanged.** The host's `TD5_NetRaceConfig.rng_seed` rides the DXPSTART payload; InitRace step 0 seeds from it on every machine (takes precedence over the trace fixed seed), and the race schedule re-seeds before the AI car fill. Verified by a 2-instance loopback run: 600 sim ticks, pose/motion/track/progress CSVs byte-identical across host and client.
...
- **Fixed (S31)**: the historical double-ACK_REPLY on DXPSTART is gone (single reply). Two lockstep deadlock races also fixed: the client now ARMS its frame-ack wait *before* sending (a low-latency broadcast could land before a post-send ResetEvent and be thrown away), and the host wipes `s_player_sync_received[]` *before* broadcasting (a fast client's next frame could beat a post-broadcast wipe and be erased). Either race stalled both sides to the 20 s timeout - reproduced and fixed via the loopback harness.
...
Dev harness: `TD5RE_NET_SELFTEST=host|join` (+`_IP`/`_PW`/`_MAX`) runs the Direct-IP join handshake headlessly (`main.c:515`, dev builds); `TD5RE_NET_LOBBY=1` boots a host lobby via `--StartScreen=11`, `TD5RE_NET_LOBBY=join` boots a client that joins 127.0.0.1, and `TD5RE_NET_AUTOSTART=1` makes the host fire START once a client joins (`td5_fe_net.c`) - together these run a full 2-instance loopback race headlessly. Logs under tag `net` -> `log/engine.log`.
