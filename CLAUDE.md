...
**Code map first.** `codemap/*.tsv` (gitignored, regenerated by every
`build_all.bat`; manually: `python scripts/gen_codemap.py` — run that if the
directory is missing/stale) answers location questions with ONE grep on a
small file instead of a fan-out over ~145k LOC:

| File | One row per | Ask it |
|------|------------|--------|
| `codemap/functions.tsv` | function (file, line, static/public, name, LOC, desc) | "where is X / how big / what's near it" |
| `codemap/sections.tsv` | `/* ==== */` banner + `SECTION:` anchor (file, line, title) | "give me a Read offset into a giant module" |
| `codemap/xref_g_td5.tsv` | (g_td5 field, file, reads, writes) | "who reads/WRITES this state" |
| `codemap/includes.tsv` | `#include ""` edge | "who includes what" |
| `codemap/statics.tsv` | file-scope mutable static | "where does this module hide state" |

**Routing table — symptom → where to look:**

| Symptom / topic | Start at |
|-----------------|----------|
| Camera (chase, countdown fly-in, spline, spectate) | `td5_camera.c` (presets: `td5_camera_profiles.h`; fly-in gate: `td5_camera_flyin_enabled`) |
| Menu screens, layout, navigation | `FRONTEND_SCREEN_GUIDE.md` FIRST; screen table `td5_frontend.c` + `TD5_ScreenIndex` in `td5_types.h`; per-area `td5_fe_*.c` |
| Race HUD, minimap, pause overlay, split-screen panes | `td5_hud.c` (SECTION anchors: filler pane, vector minimap) |
...
| "What did the ORIGINAL do?" | grep `re/ghidra_export/` (see RE section below) |
| Architecture direction / where does new state go | `STRUCTURE_ROADMAP.md` + the g_td5 ownership contract in `td5re.h` |

## Source port modules

The table below is GENERATED from `srcs.txt` + each module's own header
comment — regenerate after adding/splitting modules with
`pwsh scripts/gen_module_table.ps1 -Write`. Never hand-edit between the markers.

<!-- BEGIN MODULE TABLE (generated by scripts/gen_module_table.ps1 - do not hand-edit) -->
| File | Responsibility |
|------|---------------|
...
| `codebase-memory` | Cross-reference source port (2286 nodes; index predates the 2026-07 module splits — re-index before trusting, or prefer `codemap/*.tsv` which every build refreshes) | Navigation across source modules |
