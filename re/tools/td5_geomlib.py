...
def cmd_prefabs(root, out, level, entries, kinds, min_faces, hdr_path, man_path):
    ...
    nv = sum(len(g["local"]) for _r, g in prefabs)
    with open(hdr_path, "w", newline="\n", encoding="utf-8") as f:
        ...
        for i, (r, g) in enumerate(prefabs):
            sym = "k_pf%d" % i
            f.write("/* %s  %s  %d faces, %d verts, %d cmds, "
                    "footprint %.0fx%.0f, height %.0f */\n"
                    % (r["id"], r["kind"], r["faces"], len(g["local"]),
    ...
    for r, g in prefabs[:10]:
        print("    %-16s %-9s faces=%-4d %.0fx%.0f h=%.0f"
              % (r["id"], r["kind"], r["faces"], g["fx"], g["fz"], g["height"]))
