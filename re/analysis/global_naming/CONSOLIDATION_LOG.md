...
## batch_39_p6_struct_globals_reconcile applied 2026-06-02: 3 renames + 0 function-renames + 4 comments + 0 skipped. Backup TD5.rep.bak.20260602-002216-pre-apply. Renames (all _PROVISIONAL→confirmed, drop suffix): 0x00495234 g_extrasGalleryEnabledFlag (brief's "g_cpuHasMmx duplicate" did NOT exist — single symbol), 0x004aee14 g_trackCircuitFlagOrLapCount_PROVISIONAL→g_trackLightingZoneTablePtr (disproven guess corrected), 0x004aee54 g_modelsDatPostEntryTableBlob (brief said "...End"; real name "...Blob"). Comment-only: 0x004962a4 g_tgaLoadFailureLatch DEAD (15W/0R). Header td5_orig_globals.h NOT updated (P6 brief = no port edit). Pool sync: slot0 refreshed, slot1 hit Device-or-resource-busy/.gbf incident, slots2-15 pending — re-run sync after reboot. Also fixed a set -e bug in scripts/ghidra_pool.sh (((x++)) → x=$((x+1))) that aborted sync after the first slot.
